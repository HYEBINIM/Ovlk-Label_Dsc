...
def update_assy3read(id, is_lh=True):
    try:
        ...
        print(f"[{get_current_time()}]Update Error: {str(e)}")
    finally:
        cursor.close()
        conn.close()
...
def get_lh_check():
    ...
    try:
        with app.app_context():  # 애플리케이션 컨텍스트 설정
            ...
            # print(f"[{get_current_time()}]LH 프린팅 신호 : {result[0]}")
        ...
        print(f"[{get_current_time()}]LH DB연결 및 프린팅신호 수신 실패 : {str(e)}")
    finally:
        cursor.close()
        conn.close()
...
def get_rh_check():
    ...
    try:
        with app.app_context():  # 애플리케이션 컨텍스트 설정
            ...
            result = cursor.fetchone()
            # print(f"[{get_current_time()}]RH 프린팅 신호 : {result[0]}")
        ...
        print(f"[{get_current_time()}]RH DB연결 및 프린팅신호 수신 실패: {str(e)}")
    finally:
        cursor.close()
        conn.close()
    ...
    try:
        ...
        if res:
            ...
            if lh_count is not None:
                print(f"[{get_current_time()}]카운트: {int(lh_count)} 제품: {lh_code} 지그: {lh_zig}")
            else:
                print("lh_count가 None입니다.")

            print(f"[{get_current_time()}]LH스캔: {lh_scan}")
            ...
            try:
                ...
                if res3:
                    ...
                    print(f"[{get_current_time()}]토크1: {lh_torq1} 토크2: {lh_torq2}")

                    return lh_scan, lh_count, lh_code, lh_zig, lh_torq1, lh_torq2
            except Exception as e:
                print(f"[{get_current_time()}]Database Error: {str(e)}")
    ...
    try:
        ...
        if res:
            ...
            if rh_count is not None:
                print(f"[{get_current_time()}]카운트: {int(rh_count)} 제품: {rh_code} 지그: {rh_zig}")
            else:
                print("rh_count가 None입니다.")

            print(f"[{get_current_time()}]rh스캔: {rh_scan}")
            ...
            try:
                ...
                if res3:
                    ...
                    print(f"[{get_current_time()}]토크1: {rh_torq1} 토크2: {rh_torq2}")
...
def print_lh(lh_data):
    try:
        ...
        ^XZ
        """
        print(f"[{get_current_time()}]출력토크1: {lh_torq1_str} 출력토크2: {lh_torq2_str}")
...
def print_rh(rh_data):
    ...
    try:
        ...
        print(f"[{get_current_time()}]출력토크1: {rh_torq1_str} 출력토크2: {rh_torq2_str}")
