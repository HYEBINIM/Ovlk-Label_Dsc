...
def get_lh_num():
    try:
        ...
        if result:
            ...
            print(f"[{get_current_time()}]LH인덱스: {lh_id} 지그: {lh_zig}")
            try:
                ...
                if result_lh2:
                    ...
                    print(f"[{get_current_time()}]LH제품코드: {lh_code} 출력카운트: {(int(lh_count) + 1)}")

                    return lh_code, lh_count, lh_id, lh_zig  # 모든 값 반환
            except Exception as e:
                print(f"[{get_current_time()}]index_code에서 LH 제품 찾을 수 없음: {str(e)}")
...
def get_rh_num():
    try:
        ...
        if result:
            ...
            print(f"[{get_current_time()}]RH인덱스: {rh_id} 지그: {rh_zig}")
            ...
            try:
                ...
                if result_rh2:
                    ...
                    print(f"[{get_current_time()}]RH제품코드: {rh_code}  출력카운트: {rh_count}")

                    return rh_code, rh_count, rh_id ,rh_zig  # 모든 값 반환
            except Exception as e:
                print(f"[{get_current_time()}]index_code에서 RH 제품 찾을 수 없음: {str(e)}")
                return None, None, None, None
        return None, None, None, None
    except Exception as e:
        print(f"[{get_current_time()}]Database Error: {str(e)}")
